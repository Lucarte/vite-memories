	...
	return (
		<Popover
			...
			className={({ open }) =>
				classNames(
					open
						? "flex flex-col gap-4 p-10 fixed inset-0 z-40 overflow-y-auto"
						: "",
					"bg-white"
				)
			}
			...
			{({ open }) => (
				<>
					<div className='relative flex justify-between pr-2 lg:gap-8'>
						{/* Logo */}
						<div
							className={classNames("flex items-center flex-shrink-0", {
								"opacity-0": open,
							})}>
							<a href='#'>
								<svg
						...
						{/* Search Icon */}
						<div className='flex py-4 pr-6 md:pr-24'>
						...
						<div className='flex items-center md:absolute md:inset-y-0 md:right-0'>
							<PopoverButton className='relative inline-flex items-center justify-center p-2 -ml-2 text-black rounded-md hover:bg-gray-900 hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-orange-500'>
								<span className='-inset-0.5' />
								<span className='sr-only'>Open menu</span>
								{open ? (
									// <XMarkIcon className='block w-6 h-6' aria-hidden='true' />
									<h1 className='text-2xl font-black'>X</h1>
								) : (
